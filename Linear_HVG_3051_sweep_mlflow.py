"""
Linear baseline hyperparameter sweep for Jurkat Perturb-seq prediction.

This script uses the SAME data setup as the MLP HVG sweep:
...
SWEEP_OUTPUT_DIR = Path("outputs") / "linear_sweep"
...
def prepare_linear_style_data():
    """
    Reproduce the same data setup used by the linear baseline and MLP sweep:
    - HVG only
...
def run_one_experiment(
    ...
    run_name = (
        f"linear_sweep_lr{learning_rate}_"
    ...
    safe_lr = format_float_for_filename(learning_rate)
    safe_wd = format_float_for_filename(weight_decay)

    run_file_prefix = f"linear_lr{safe_lr}_batch{batch_size}_wd{safe_wd}"
    ...
    print(f"Linear architecture: {INPUT_DIM} -> {OUTPUT_DIM}")
    print("=" * 80)

    with mlflow.start_run(run_name=run_name):
        mlflow.log_param("model_type", "linear")
        mlflow.log_param("run_group", "linear_hvg_3051_hyperparameter_sweep")
        ...
        config = {
            ...
            "run_group": "linear_hvg_3051_hyperparameter_sweep",
            "data_setup": "same_as_linear_hvg_3051",
            "input_dim": INPUT_DIM,
            "output_dim": OUTPUT_DIM,
            "num_genes": NUM_GENES,
            "num_perturbations": NUM_PERTURBATIONS,
            "architecture": f"{INPUT_DIM} -> {OUTPUT_DIM}",
...
def main():
    ...
    # Save one fixed test set using batch size 64 only for re-evaluation convenience.
    # The test samples are the same regardless of batch size.
    _, _, fixed_test_loader = build_dataloaders(
        ControlData,
        TrainingData,
        ValidationData,
        TestData,
    ...
    x_test_path, y_test_path, X_test, y_test = save_test_tensors(fixed_test_loader)
    ...
    sweep_combinations = list(
        itertools.product(LEARNING_RATES, BATCH_SIZES, WEIGHT_DECAYS)
    )

    print("\nTotal linear sweep runs:", len(sweep_combinations))
    ...
    for learning_rate, batch_size, weight_decay in sweep_combinations:
    ...
    results_path = SWEEP_OUTPUT_DIR / "linear_hvg_3051_sweep_summary.csv"
    results_df.to_csv(results_path, index=False)

    best_row = results_df.sort_values("test_loss", ascending=True).iloc[0]

    print("\n" + "=" * 80)
    print("LINEAR SWEEP FINISHED")
    print("=" * 80)
    print("Best run by test_loss:")
    print(best_row)
    print("\nSummary saved to:", results_path)

    with mlflow.start_run(run_name="linear_hvg_3051_sweep_summary"):
        mlflow.log_param("run_group", "linear_hvg_3051_hyperparameter_sweep_summary")
        ...
        mlflow.log_param("best_weight_decay", float(best_row["weight_decay"]))
    ...
    print("Logged linear sweep summary to MLflow.")
