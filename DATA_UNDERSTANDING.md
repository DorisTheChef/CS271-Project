...
- **Differential Expression Warnings:** When computing differential expression (e.g., via `sc.tl.rank_genes_groups` across all 68 perturbations), two common warnings occur:
    - **Few Observations:** Some perturbations have very few cells (e.g., `RRM1` with 3, `HMGCR` with 8). Scanpy's Wilcoxon test uses a normal approximation for p-values, which flags warnings for groups with $\le 25$ cells.
    - **Pandas Fragmentation:** Computing comparisons for 68 groups against the control iteratively builds a results DataFrame with hundreds of columns, triggering `pandas.errors.PerformanceWarning`. These warnings are expected and should be explicitly suppressed during analysis.
